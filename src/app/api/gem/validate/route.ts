...
export const POST = async (request: Request): Promise<NextResponse> => {
  ...
  try {
    ...
    console.log("🚀 Gem validation endpoint called")
    console.log(`🗑️ Removing ${invalidJobIds.length} invalid Gem jobs`)

    // Delete the invalid jobs
    await db.delete(jobs).where(inArray(jobs.id, invalidJobIds))

    console.log(`✨ Removed ${invalidJobIds.length} invalid Gem jobs`)
    ...
    console.error("❌ Gem validation failed:", err)
    return NextResponse.json(
      {
        success: false,
        error: errorMsg,
