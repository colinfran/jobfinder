...
export const POST = async (request: Request): Promise<NextResponse> => {
  ...
  try {
    ...
    console.log("🚀 Workday validation endpoint called")
    console.log(`🗑️ Removing ${invalidJobIds.length} invalid Workday jobs`)

    // Delete the invalid jobs
    await db.delete(jobs).where(inArray(jobs.id, invalidJobIds))

    console.log(`✨ Removed ${invalidJobIds.length} invalid Workday jobs`)
    ...
    console.error("❌ Workday validation failed:", err)
    return NextResponse.json(
      {
        success: false,
        error: errorMsg,
