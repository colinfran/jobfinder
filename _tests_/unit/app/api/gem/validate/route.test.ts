...
import { POST } from "@/app/api/gem/validate/route"
...
describe("api/gem/validate POST", () => {
  ...
  it("returns 401 when authorization header is invalid", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/gem/validate", { method: "POST" }),
    )
  ...
  it("returns 400 when invalidJobIds is missing or empty", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/gem/validate", {
        method: "POST",
  ...
  it("deletes invalid jobs and returns summary", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/gem/validate", {
        method: "POST",
        headers: { authorization: "Bearer secret", "content-type": "application/json" },
        body: JSON.stringify({ invalidJobIds: [2, 4] }),
      }),
    )
